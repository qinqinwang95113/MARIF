from utils import ML1M, convert_unique_idx, AllFeaturePair_Constant_id_Sample
...
users_df = pd.read_csv('../dataset/ml-1m/users.dat', sep='::', names=['user_id', 'gender', 'age', 'occupation', 'zip'], engine = "python")
...
ratings_df = pd.read_csv('../dataset/ml-1m/ratings.dat', sep='::', names=['user_id', 'item_id', 'rating', 'timestamp'], engine = "python")
...
assert len(np.intersect1d(rest_df["item_id"].unique().tolist(), train_df["item_id"].unique().tolist())) == rest_df["item_id"].nunique()
